...
const Header = () => {
  ...
  return (
    <header
      className={`w-full flex items-center justify-between   dark:border-gray-200 bg-[#EAEDFB] border-black border-[1px] m-[0.5px] dark:bg-gray-700 flex-col md:flex-row`}>
      <div className={`flex justify-between w-full ${menu && "md:w-1/2"}`}>
        <img
          src={logo}
          alt=""
          className="mix-blend-darken dark:mix-blend-multiply border-none w-42 h-14 ml-4 rounded-xl"
        />
        ...
        <button
          ...
          className="md:hidden">
          <img className="text-xs w-24 mr-4 h-10" src={burgerMenu} />
        </button>
      </div>
      <Navbar menu={menu} />
    </header>
